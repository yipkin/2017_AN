...
void analyze(UInt_t RunNumber=18176018) {
  ...
  Bool_t BCounted = kFALSE ;
  int iboth = 0 ;

  Int_t nafter_theta3sigma = 0, ngoodfit = 0, nfiducial = 0 ;
  ...
  */
    ...
    */
	...
	if ( cls->planeId() < LastPlane ) 
      ...
      */
      ...
      ADC[0] = rpEvt->adc(LastRP, 0); 
      ADC[1] = rpEvt->adc(LastRP, 1); 
      TAC[0] = rpEvt->tac(LastRP, 0); 
      TAC[1] = rpEvt->tac(LastRP, 1); 


      //      if ( ADC[0] > 0 || ADC[1] > 0 )
      //	cout << "ADC : " << ADC[0] << " , " << ADC[1] << endl ;

      //      if ( TAC[0] > 0 || TAC[1] > 0 )
      //	cout << "TAC : " << TAC[0] << " , " << TAC[1] << endl ;
  ...
  cout << "No. of events after Fiducial Cuts : " << nfiducial << endl ;
